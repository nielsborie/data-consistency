...
def reference_consistency(df1, df2, id_column_name):
    try:
        ...
        same_id = np.unique(df1[id_column_name].fillna("0").astype("str")) == np.unique(df2[id_column_name].fillna("0").astype("str"))
...
def plot_gantt(df1: pd.DataFrame, df2: pd.DataFrame, date_col: str):
    gantt_list = []
    periods = []
    for i, dd in enumerate([df1, df2]):
        start_date = min(pd.to_datetime(dd[date_col])).to_pydatetime().strftime('%Y-%m-%d')
        end_date = max(pd.to_datetime(dd[date_col])).to_pydatetime().strftime('%Y-%m-%d')
